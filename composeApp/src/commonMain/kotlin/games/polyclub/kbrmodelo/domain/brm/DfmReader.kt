...
/**
 * Parses a brModelo `.brM` file (Delphi binary DFM stream) into a [DfmNode] tree.
 *
 * ## File structure
 * ```
 ...
 * ```
 * Property value types correspond to Delphi's `TValueType` enum (see [DfmValue]).
 *
...
private class DfmByteReader(private val data: ByteArray) {
    ...
    fun readComponent(): DfmNode {
        ...
        return DfmNode(className, instanceName, props, children)
    }
        ...
        return when (val type = readUByte()) {
            ...
            0x0f -> {
                ...
                if (charCount <= 0) {
                    ...
                    DfmValue.StrVal(decodeUtf16LE(raw))
                }
            }
