...
import games.polyclub.power.brmodelo.domain.xml.*
import games.polyclub.power.brmodelo.domain.xml.boolValor
import games.polyclub.power.brmodelo.domain.xml.textNode
import games.polyclub.power.brmodelo.domain.xml.valor

/**
 * Serializes a [games.polyclub.power.brmodelo.domain.ConceptualSchema] to the brModelo XML format.
 *
...
object ConceptualSchemaXmlSerializer {
    ...
    private fun buildInformacoes(schema: ConceptualSchema, totalItens: Int): XmlNode =
        xmlNode("Informacoes") {
            ...
            add(
                xmlNode(
                    "Observacao",
                    "Valor" to schema.observations
                )
            )
            add(buildDefaultTemplate())
        }

    private fun buildDefaultTemplate(): XmlNode =
        xmlNode("Template") {
            add(xmlNode("CAMPOS"))
            add(xmlNode("TIPOS"))
            ...
            add(xmlNode("DDL") {
                ...
                add(
                    xmlNode(
                        "CCamp",
                        text = "*\$nome_campo *\$tipo_campo *\$compl_campo"
                    )
                )
    ...
    private fun buildAtributo(
        ...
        return xmlNode(
            ...
            valor("Orientacao", attr.labelSide.pascalXmlCode)
    ...
    private fun buildAutoRelacoes(
        ...
        return xmlNode(
            "AutoRelacoes",
            "AutoRelacionado" to if (selfRels.isNotEmpty()) -1 else 0
        ) {
    ...
    private fun buildEspecializacoes(
        ...
        return xmlNode(
            "Especializacoes",
            "ehEsp" to if (specs.isNotEmpty()) -1 else 0
        ) {
     ...
     *
     * Each [games.polyclub.power.brmodelo.domain.HiddenAttribute] is serialized as `<AtributoOculto Nome="...">` with its
    ...
    private fun buildAtributoOculto(attr: HiddenAttribute): XmlNode =
        xmlNode("AtributoOculto", "Nome" to attr.name) {
            add(
                xmlNode(
                    "LeftTop",
                    "X" to attr.position.x,
                    "Y" to attr.position.y
                )
            )
            valor("MaxCard", attr.cardinality.maxCardinality)
            valor("MinCard", attr.cardinality.minCardinality)
